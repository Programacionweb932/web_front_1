...
const AdminUpdateTicket = () => {
  ...
  const fetchTicket = async () => {
    try {
      const result = await axios.get(`https://web-back-1.vercel.app/api/ticket/${ticketId}`);
  ...
  const updateTicket = async () => {
    try {
      await axios.put(`https://web-back-1.vercel.app/api/ticket/${ticketId}`, { status, response });
